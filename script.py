...
def pad_quotes_with_spaces(data):
    """
    """
    data = re.sub('"', ' " ', data)
    return data

def pad_commas_with_spaces(data):
    """
    """
    data = re.sub('،', ' ، ', data)
    return data
...
def preprocess_text(df, text_column_name="law_akn_text"):
    ...
    df[text_column_name] = df[text_column_name].apply(lambda x: pad_quotes_with_spaces(x))
    df[text_column_name] = df[text_column_name].apply(lambda x: pad_commas_with_spaces(x))
...
def save_text_files(texts_df, name=""):
    ...
    base_file_path = f"output_files/{name}_{now_str}"
...
def ocr_predict_errors_pipline(docx_path, name_to_save=""):
    tokenizer = load_tokenizer("./serialized/tokenizer.pickle")
    model = get_model(len(tokenizer.word_index) + 1)
    model = keras.models.load_model("models/try1_weights.12-0.09309.hdf5",
